...
export default function AlertsPage() {
  ...
  if (loading) {
    return <div className="max-w-4xl mx-auto px-4 py-10 text-sm text-muted-foreground">Loading…</div>;
  }
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-10">
        <Card className="p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Sign in to save searches and get in-app alerts when new titles match.</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10 space-y-8">
      ...
      <section className="space-y-3">
        ...
        {alerts.length === 0 ? (
          <Card className="p-5 text-sm text-muted-foreground">No alerts yet. Save a search from the search page.</Card>
        ) : (
      ...
      <section className="space-y-3">
        ...
        {isLoading ? (
          ...
          <Card className="p-5 text-sm text-muted-foreground">
            No saved searches yet. Use <Link to="/search" className="text-primary hover:underline">Search</Link> and click Save search.
          </Card>
        ) : (
          <div className="space-y-2">
            {searches.map((search) => (
              <Card key={search.id} className="p-4 space-y-2">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{parseFilters(search.filters_json)}</p>
                  </div>
                  <div className="flex items-center gap-1">
                    <button
                      type="button"
                      onClick={() => toggleMutation.mutate({ id: search.id, paused: !search.paused })}
                ...
                <p className="text-[11px] text-muted-foreground">
                  {search.paused ? "Paused" : "Active"} · last checked {search.last_checked_at ?? "never"}
                </p>
              </Card>
            ))}
          </div>
