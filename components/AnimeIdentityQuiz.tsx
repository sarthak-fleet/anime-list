...
const QUESTIONS: Question[] = [
  ...
  },
  {
    ...
    answers: [
      { label: "Mind games and tradeoffs", weights: { "steady-strategist": 2, "world-builder": 1 } },
  ...
  },
  {
    ...
    answers: [
      { label: "A tight 12-26 episode run", weights: { "quiet-observer": 2, "steady-strategist": 1 } },
      { label: "A long journey if the cast is worth it", weights: { "world-builder": 2, "chaotic-optimist": 1 } },
      ...
      <section className="space-y-4">
        {QUESTIONS.map((question, index) => (
          <div key={question.id} className="rounded-xl border border-border bg-card p-5">
            <p className="text-xs font-medium uppercase text-muted-foreground">Question {index + 1}</p>
      ...
      <aside className="h-fit rounded-xl border border-border bg-card p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {result.traits.map((trait) => (
            <span key={trait} className="rounded-full border border-border px-3 py-1 text-xs text-muted-foreground">
              {trait}
            </span>
          ))}
        </div>
        ...
        {!complete && (
          <p className="mt-3 text-xs text-muted-foreground">Answer all questions to unlock the search link.</p>
        )}
      </aside>
    </div>
  );
